...
from utils.validate_ex import algorithm_validate_he
        ...
        for i, (index, img, label) in enumerate(train_loader):
            ...
            if label.shape[0]<2:
                ...
                if iter_num % args.test_iter == 0:
                    if args.data == 'he':
                        val_multi_acc, val_multi_f1 = algorithm_validate_he(
                        ...
                        if (val_multi_acc+val_multi_f1) > best_val_auc:
                            if epoch>10:
                                ...
                                test_multi_acc, test_multi_f1= algorithm_validate_he(model, domain1_loader, epoch, 'test',
                                                                             device,writer=writer)
                                print("The test results are ", test_multi_acc, " ", test_multi_f1)
                                # torch.save(model.state_dict(), args.save_path)

                iter_num = iter_num + 1
    if args.data == 'he':
        val_multi_acc, val_multi_f1 = algorithm_validate_he(model,val_loader,epoch, 'val',device,args.save_path,writer=writer)
        val_multi_acc= algorithm_validate_he(model, domain1_loader, epoch, 'test', device,args.save_path,writer=writer )
...
if __name__ == '__main__':
    ...
    # python main_bracs.py --data he --source_domains APT --algorithm resnet18-MIX-SP --K 5 --ratio 0.8 --alpha 5.0 --beta 3.0 --N_Times 20 --device cuda:0
