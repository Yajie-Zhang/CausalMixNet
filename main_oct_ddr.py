...
def get_args_parser():
    ...
    parser.add_argument('--source_domains',default='DDR',type=str)
            ...
            if label.shape[0]<2:
                ...
                if iter_num % args.test_iter == 0:
                    if args.data == 'covid':
                        ...
                        if (val_acc+val_f1+val_auc) > best_val_auc:
                            best_val_auc = val_acc+val_f1+val_auc
                            test_acc, test_f1, test_auc = algorithm_validate(model, test_loader, 0, 'test', device,
                                                                         writer=writer)
                            print('=============the best results============')
                            print(test_acc, test_auc, test_f1)
...
def main(args):
    ...
    if args.data=='covid':
        ...
        args.save_path = './result/' + args.algorithm + '_'+args.data+'_' + args.source_domains
...
if __name__ == '__main__':
    ...
    # python main_oct_ddr.py --data covid --source_domains DDR --device cuda:1 --algorithm resnet18-MIX-SP --K 5 --ratio 0.8 --alpha 0.5 --N_Times 20
