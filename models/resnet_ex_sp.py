...
class Model(nn.Module):
    ...
    def cal_binary_last_layers(self,x):
        x = self.layer4(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        y = self.binary_cls(x)
    ...
    def forward(self, x,pro_att_ori=None,label_oh=None,N=3):
        ...
        if label_oh is not None:
            ...
            # _, ex_binary_y=self.cal_binary_last_layers(ex_x_3+x_3)
            # _, sim_binary_y=self.cal_binary_last_layers(sim_ex_x_3+x_3)
...
class ModelBinary(nn.Module):
    ...
    def forward(self, x,pro_att_ori=None,label_oh=None,N=3):
        ...
        if label_oh is not None:
            ...
            # ex_x_3=ex_x_3
            _,_,ex_x_3=self.CrossAtt(x_3,ex_x_3)
            ex_x,ex_y=self.cal_binary_last_layers(ex_x_3+x_3)

            # _, _, sim_ex_x_3 = self.CrossAtt(x_3, sim_ex_x_3)

            _, _, sim_ex_x_3 = self.CrossAtt(sim_ex_x_3,x_3)
            sim_ex_x, sim_ex_y = self.cal_binary_last_layers(sim_ex_x_3 + x_3)

            _, multi_ex_y=self.cal_few_last_layers(ex_x_3+x_3)
            _, multi_sim_y=self.cal_few_last_layers(sim_ex_x_3+x_3)

            return x_sim,x,y,sim_ex_x, sim_ex_y,ex_x,ex_y,y_multi, multi_ex_y, multi_sim_y
        return x_4,x,y
