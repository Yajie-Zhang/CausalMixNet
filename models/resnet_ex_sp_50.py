...
from torchvision.models import resnet,alexnet,densenet121
...
class Model(nn.Module):
    def __init__(self, num_classes=1000,mode='single_cls',dim=2048,K=100):
        ...
        self.CrossAtt=NonLocalAttention(1024)
        self.dim=dim
    ...
    def forward(self, x,pro_att_ori=None,label_oh=None,N=3):
        ...
        if label_oh is not None:
            ...
            for i in range(x.shape[0]):
                ...
                cur_assembly = (assembly[i]).view(1, 16 * 16, 1024).repeat(16, 1, 1)
                cur_masked = (cur_masked.matmul(cur_assembly)).permute(0, 2, 1).view(16, 1024, 16, 16)
            ...
            masked_x3_set = masked_x3_set.view(-1, 1024, 16, 16)
            # mask_x3_set=torch.stack(mask_x3_set)
            # mask_x3_set=mask_x3_set.view(-1,256,16,16)
            masked_x, masked_y = self.cal_few_last_layers(masked_x3_set+x_3.view(x.shape[0],1,1024,16,16).repeat(1,16,1,1,1).view(-1,1024,16,16))
            ...
            masked_x_view=masked_x.view(label_oh.shape[0],16,2048)
            x_view=x.view(label_oh.shape[0],1,2048).repeat(1,16,1)
            ...
            for i in range(x.shape[0]):
                ...
                cur_x3=(x_3_clone[i] ).view(1024,16*16).permute(1,0)  #16*16,256
                cur_ex_x3=pair_x3[i].view(1024,16*16).permute(1,0)

                cur_x3[x3_mask]=cur_ex_x3[ex_x3_mask]
                cur_x3=cur_ex_x3.permute(1,0).view(1024,16,16)
                ex_x_3[i]=cur_x3
                ...
                cur_x3 = (x_3_clone[i]).view(1024, 16 * 16).permute(1, 0)  # 16*16,256
                cur_ex_x3 = sim_pair_x3[i].view(1024, 16 * 16).permute(1, 0)
...
class Model1(nn.Module):
    def __init__(self, num_classes=1000,mode='single_cls',dim=1024,K=100):
        super(Model1, self).__init__()
        self.num_classes=num_classes
        # ResNet=resnet.resnet50(pretrained=True)
        ResNet=densenet121(pretrained=True)
        self.layer1=ResNet.features[0:11]
        self.layer4=ResNet.features[11]
        self.CrossAtt=NonLocalAttention(1024)
        self.dim=dim
        self.cls = nn.Linear(self.dim,num_classes)
        self.K=K
    ...
    def forward(self, x,pro_att_ori=None,label_oh=None,N=3):
        ...
        x = F.adaptive_avg_pool2d(x_4, (1, 1))
        x = torch.flatten(x, 1)
        y = self.cls(x)
        if label_oh is not None:
            mask = (self.patch_set()).to(x.device)
            ...
            # mask_x3_set=[]
            for i in range(x.shape[0]):
                cur_score = (score[i]).view(1, 8 * 8, 8 * 8).repeat(16, 1, 1)
                cur_masked = cur_score * (1 - mask)
                # cur_mask=cur_score*mask
                cur_masked = torch.softmax(cur_masked, dim=2)
                # cur_mask=torch.softmax(cur_mask,dim=2)
                cur_assembly = (assembly[i]).view(1, 8 * 8, 1024).repeat(16, 1, 1)
                cur_masked = (cur_masked.matmul(cur_assembly)).permute(0, 2, 1).view(16, 1024, 8, 8)
                # cur_mask=(cur_mask.matmul(cur_assembly)).permute(0,2,1).view(16,256,16,16)
                masked_x3_set.append(cur_masked)
                # mask_x3_set.append(cur_mask)
            masked_x3_set = torch.stack(masked_x3_set)  # shape=B,16,256,16,16
            masked_x3_set = masked_x3_set.view(-1, 1024, 8, 8)
            # mask_x3_set=torch.stack(mask_x3_set)
            # mask_x3_set=mask_x3_set.view(-1,256,16,16)
            masked_x, masked_y = self.cal_few_last_layers(
                masked_x3_set + x_3.view(x.shape[0], 1, 1024, 8, 8).repeat(1, 16, 1, 1, 1).view(-1, 1024, 8, 8))
            # mask_x,mask_y=self.cal_few_last_layers(mask_x3_set)
            label_repeat = label_oh.view(label_oh.shape[0], 1, -1).repeat(1, 16, 1).view(-1, label_oh.shape[1])
            masked_x_view = masked_x.view(label_oh.shape[0], 16, 1024)
            x_view = x.view(label_oh.shape[0], 1, 1024).repeat(1, 16, 1)
            x_sim = ((masked_x_view - x_view) ** 2).sum(2)
            # print('x_sim',x_sim)
            ...
            for i in range(x.shape[0]):
                ...
                cur_x3 = (x_3_clone[i]).view(1024, 8 * 8).permute(1, 0)  # 16*16,256
                cur_ex_x3 = pair_x3[i].view(1024, 8 * 8).permute(1, 0)

                cur_x3[x3_mask] = cur_ex_x3[ex_x3_mask]
                cur_x3 = cur_ex_x3.permute(1, 0).view(1024, 8, 8)
                ex_x_3[i] = cur_x3
                ...
                cur_x3 = (x_3_clone[i]).view(1024, 8 * 8).permute(1, 0)  # 16*16,256
                cur_ex_x3 = sim_pair_x3[i].view(1024, 8 * 8).permute(1, 0)
