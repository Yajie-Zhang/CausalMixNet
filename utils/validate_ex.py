    ...
    with torch.no_grad():
        ...
        if val_type == 'val':
            ...
            if val_type in ['val', 'test']:
                ...
                logging.info(
                    '{} - epoch: {}, multi_acc: {}, multi_f1: {}.'.format
                    (val_type, epoch, multi_acc, multi_f1))
            algorithm.train()
            return multi_acc,multi_f1
        else:
            multi_acc = accuracy_score(label, pred)
            multi_f1 = f1_score(label, pred, average='macro')
...
def algorithm_validate_he_binary(algorithm, data_loader, epoch, val_type, device,best_model=None,writer=None):
    if best_model is not None:
        path = torch.load(best_model, map_location=device)
        algorithm.load_state_dict(path, strict=True)
    algorithm.eval()
