...
def algorithm_validate(algorithm, data_loader, epoch, val_type, device,best_model=None):
    ...
    with torch.no_grad():
        ...
        if val_type in ['val', 'test']:
            print('{} - epoch: {}, loss: {}, acc: {}, auc: {}, F1: {}.'.format
...
def algorithm_validate_he(algorithm, data_loader, epoch, val_type, device,best_model=None):
    if best_model is not None:
        path = torch.load(best_model, map_location=device)
        algorithm.load_state_dict(path, strict=True)
    algorithm.eval()
    ...
    with torch.no_grad():
        ...
        if val_type == 'val':
            ...
            pred_error_index = np.where(pred == 2)[0]
            if pred_error_index.shape[0] > 0:
                pred[pred_error_index] = 1 - label[pred_error_index]
...
def algorithm_validate_he_break(algorithm, data_loader, epoch, val_type, device,best_model=None):
    if best_model is not None:
        path = torch.load(best_model, map_location=device)
        algorithm.load_state_dict(path, strict=True)
    algorithm.eval()
...
def mean_average_precision(query_loader, base_loader, args):
    if best_model is not None:
        path = torch.load(best_model, map_location=device)
        algorithm.load_state_dict(path, strict=True)
    device = torch.device(args.device)
    d_ft = 512 * (1 + args.num_att)
    algorithm.eval()
    with torch.no_grad():
        query_code = torch.zeros(len(query_loader.dataset), d_ft)
        query_label = torch.zeros(len(query_loader.dataset), 200)
        ...
        for ind, image, label in query_loader:
            ...
            ft, _, output = algorithm(image)
            query_code[ind] = ft.data
            query_label[ind] = label
        retrieval_code = torch.zeros(len(base_loader.dataset), d_ft)
        retrieval_label = torch.zeros(len(label_loader.dataset), 200)
